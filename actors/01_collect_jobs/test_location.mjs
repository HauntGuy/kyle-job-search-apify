...
function normalizeLocationFields(rawLocation) {
  ...
  if (segments.length >= 2) {
    // Multi-segment (comma-separated): positional parsing, right-to-left
...
// --- Tests ---
// Format: [input, expectedLocation, expectedCommutable, expectedWorkMode]
const tests = [
  // Foreign with comma
  ['Cambridge, UK',              'GBR',              false, ''],
  ['London, UK',                 'GBR',              false, ''],
  ['Berlin, DEU',                'DEU',              false, ''],
  // Foreign without comma (single-segment token parsing)
  ['Cambridge UK',               'GBR',              false, ''],
  ['London UK',                  'GBR',              false, ''],
  ['Jakarta IDN',                'IDN',              false, ''],
  // US with state (comma positional)
  ...
  ['New York, New York',         'New York NY',      false, ''],
  // Work mode prefixes
  ['Remote | Boston, MA',        'Boston MA',        true,  'Remote'],
  ['Hybrid | Cambridge, MA',     'Cambridge MA',     true,  'Hybrid'],
  ['Remote',                     '',                 null,  'Remote'],
  ['Remote (CET ±2h)',           '',                 null,  'Remote'],
  ['Remote | Istanbul, Turkey',  'TUR',              false, 'Remote'],
  // Domestic signal with city
  ['Vancouver, USA',             'Vancouver WA',     false, ''],  // 1 US Vancouver → resolves
  ['Boston, USA',                'Boston',           null,  ''],  // 3 US Bostons → ambiguous
  // Bare domestic
  ['USA',                        'USA',              null,  ''],
  // Bare city (no state/country/domestic signal) — conservative
  ['Vancouver',                  'Vancouver',        null,  ''],  // ambiguous US+CA
  ['Boston',                     'Boston',           null,  ''],  // ambiguous US+GB+PH
  // Foreign-only city (unambiguous)
  ['Izmir',                      'TUR',              false, ''],
  // Multi-word country/region
  ...
  // Georgia + domestic signal: confirms US state
  ['Atlanta, Georgia, United States', 'Atlanta GA',  false, ''],
  // "United States of America" (4-word domestic phrase)
  ['Irvine California 92618 United States of America', 'Irvine CA', false, ''],
  ['Washington United States of America', 'WA', false, ''],
  // 2-letter state vs foreign ISO2 disambiguation (Jakarta is not in Idaho)
