...
def main():
    if len(sys.argv) < 2:
        print('Usage:')
        print('  python tools/test_score.py <JOB_ID>              # Find job, use llmInput if available, call LLM')
    ...
    # Default mode: find job in dataset, use llmInput if available, else rebuild prompt
    job_id = sys.argv[1]
    ...
    # Show original evaluation if available
    original = job.get('evaluation', {})
    if original:
        print(f'  Original result: accept={original.get("accept")}, score={original.get("score")}')
        print(f'  Original red_flags: {original.get("red_flags", [])}')
        print(f'  filterReason: {job.get("filterReason", "")}')

    # 4. Build prompt — prefer llmInput (exact data sent to LLM), fall back to reconstruction
    llm_input = job.get('llmInput')
    if llm_input:
        print(f'\n  Using saved llmInput (exact data from scoring run)')
        job_data = llm_input
    else:
        print(f'\n  No llmInput saved — rebuilding prompt from job fields (may differ from run)')
        # Check if description needs enrichment (Built In jobs)
        desc = job.get('description') or ''
        sources = job.get('sources') or [job.get('source', '')]

        if not desc and any(str(s).startswith('builtin_') for s in sources):
            print('  Description is empty — fetching from Built In...')
    ...
    print(f'  Work mode: {job_data.get("workMode")}')
    print(f'  Salary: {job_data.get("salary") or "(none)"}')
    ...
    messages = build_prompt(rubric_text, job_data)
    ...
    if llm_input:
        ...
        print(f'LLM EVALUATION RESULT (rebuilt prompt)')
    print(f'{"="*60}')
    print(f'  Accept: {evaluation.get("accept")}')
    print(f'  Score:  {evaluation.get("score")}')
